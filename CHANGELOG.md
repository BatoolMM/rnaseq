...
- [PR #1811](https://github.com/nf-core/rnaseq/pull/1811) - Switch the default `--ribo_database_manifest` to `smr_v4.3_fast_db.fasta.gz` (SILVA 138 + Rfam, 85/90 % clustered) hosted as a per-variant asset on the SortMeRNA [v4.3.7 release](https://github.com/sortmerna/sortmerna/releases/tag/v4.3.7), replacing the eight SILVA 119-based FASTAs from `biocore/sortmerna/v4.3.4`. The `fast` variant keeps the SortMeRNA index footprint close to the previous default; `default` / `sensitive` / `sensitive_rfam_seeds` are documented as opt-in alternatives in `docs/usage.md`. Unblocks [#1354](https://github.com/nf-core/rnaseq/issues/1354) now that upstream has split the database archive ([sortmerna/sortmerna#458](https://github.com/sortmerna/sortmerna/issues/458))
