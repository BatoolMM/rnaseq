...
- [PR #1806](https://github.com/nf-core/rnaseq/pull/1806) - Pass `-k 200` to Bowtie2 by default when using `--aligner bowtie2_salmon` so downstream Salmon EM can redistribute multimapping reads; exposed as `--bowtie2_align_k` (set to `1` to restore single-best behaviour)
