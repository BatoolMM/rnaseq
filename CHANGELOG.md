...
- Pass `-k 200` to Bowtie2 by default when using `--aligner bowtie2_salmon` so that downstream Salmon alignment-mode EM can redistribute reads across overlapping transcripts (common in prokaryotic operons). Default matches the Salmon paper's benchmark settings (Patro et al. 2017) and RSEM's `--bowtie2-k` default. Exposed as a new `--bowtie2_align_k` parameter; set to `1` to restore the previous single-best-alignment behaviour
