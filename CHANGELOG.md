...
- [PR #1812](https://github.com/nf-core/rnaseq/pull/1812) - Narrow the pipeline-level test matrix by adding `--skip_pseudo_alignment` to aligner-focused tests (HISAT2, STAR-RSEM, UMI, Parabricks, Sentieon) and merging `skip_quantification_merge` into the Salmon test file, removing duplicate Salmon pseudoalignment runs that `default.nf.test` already covers
