...
- Pass `-k 100` to Bowtie2 by default when using `--aligner bowtie2_salmon` so that downstream Salmon alignment-mode EM can redistribute reads across overlapping transcripts (common in prokaryotic operons). Exposed as a new `--bowtie2_align_k` parameter; set to `1` to restore the previous single-best-alignment behaviour
